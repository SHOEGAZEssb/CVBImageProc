...
namespace CVBImageProcLib.Processing
{
	...
	public sealed class Math : FullProcessorBase
	{
		...
		private static Func<int, byte, byte> MakeCalculationFunc(MathMode mode, bool wrapAround)
		{
			...
			Func<int, byte, byte> calculationFunc = mode switch
			{
				MathMode.Add => (providedValue, inputByte) => wrapAroundFunc(inputByte + providedValue),
				MathMode.Subtract => (providedValue, inputByte) => wrapAroundFunc(inputByte - providedValue),
				MathMode.Divide => (providedValue, inputByte) =>
									{
										if (providedValue == 0)
											return inputByte;
										else
				...
				,
				MathMode.Multiply => (providedValue, inputByte) => wrapAroundFunc(inputByte * providedValue),
				_ => throw new ArgumentException("Unknown math mode", nameof(mode)),
			};
